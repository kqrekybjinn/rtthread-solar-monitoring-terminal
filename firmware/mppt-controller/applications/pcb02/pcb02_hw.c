...
rt_err_t pcb02_hw_init(const pcb02_config_t *cfg)
{
    ...
    rt_pin_mode(PCB02_PIN_GPIO17_DISABLE, PIN_MODE_INPUT);
                            ...
                            rt_uint16_t pwm_max_counts)
{
    ...
    if (s_pwm_dev != RT_NULL && s_cfg != RT_NULL && pwm_max_counts > 0)
    {
        ...
        ret = rt_pwm_set(s_pwm_dev, s_cfg->pwm_channel, period_ns, pulse_ns);
        if (ret != RT_EOK)
        {
            rt_kprintf("pcb02: pwm set failed ch=%d period=%u pulse=%u ret=%d\r\n",
                       s_cfg->pwm_channel,
                       period_ns,
                       pulse_ns,
                       ret);
        ...
        {
            ret = rt_pwm_enable(s_pwm_dev, s_cfg->pwm_channel);
            if (ret != RT_EOK)
            {
                rt_kprintf("pcb02: pwm enable failed ch=%d ret=%d\r\n", s_cfg->pwm_channel, ret);
            }
        }
        else
        {
            ret = rt_pwm_disable(s_pwm_dev, s_cfg->pwm_channel);
            if (ret != RT_EOK)
            {
                rt_kprintf("pcb02: pwm disable failed ch=%d ret=%d\r\n", s_cfg->pwm_channel, ret);
            }
        }
    }
#else
